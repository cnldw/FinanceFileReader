...
void miniblink::init()
{
    ...
    if (!isInit) {
        ...
        //不同的构建套件位数加载不同的动态库
#ifdef Q_OS_WIN64
        QString file = qApp->applicationDirPath() + "/plugin-web/miniblink_x64.dll";
#else
        QString file = qApp->applicationDirPath() + "/plugin-web/miniblink.dll";
#endif
...
{
    const char *temp = QUrl::fromLocalFile(url).toEncoded().data();
    if (file) {
