    ...
    QWidget(parent)
{
    ...
    score = new QLabel(QString("成绩: %1").arg(game->getScore()));
    score->setStyleSheet("QLabel { color: rgb(255,255,255); font: 16pt; }");
...
void QGameBoard::notify()
{
    ...
    if (game->won())
        score->setText(QString("恭喜你,成功拿到2048了,继续加油！.\t\t 成绩: %1").arg(game->getScore()));
    else
        score->setText(QString("成绩: %1").arg(game->getScore()));
