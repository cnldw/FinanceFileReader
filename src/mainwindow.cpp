    ...
    ui(new Ui::MainWindow)
{
    ...
    watcherXlsxSaveStatus = new QFutureWatcher<bool>;
    connect(watcherXlsxSaveStatus, &QFutureWatcher<bool>::finished,this, &MainWindow::save2XlsxFinished);
    watcherXlsxImportStatus = new QFutureWatcher<bool>;
    connect(watcherXlsxImportStatus, &QFutureWatcher<bool>::finished,this, &MainWindow::importFromXlsxFinished);
    watcherZipExtractStatus = new QFutureWatcher<bool>;
    connect(watcherZipExtractStatus, &QFutureWatcher<bool>::finished,this, &MainWindow::extractZipFinished);
}
...
MainWindow::~MainWindow()
{
    ...
    delete watcherXlsxSaveStatus;
    watcherXlsxSaveStatus=nullptr;
    delete watcherXlsxImportStatus;
    watcherXlsxImportStatus=nullptr;
    delete watcherZipExtractStatus;
    watcherZipExtractStatus=nullptr;
}
    ...
    if(currentFileName.endsWith(".zip",Qt::CaseInsensitive)){
        ...
        if(fileCount>0){
            ...
            if(!dialog.getConfirm()){
                ...
                watcherZipExtractStatus->setFuture(future);
                return;
            }
        }
        else{
...
bool MainWindow::updateOrBlcokCheck(){
    if(isUpdateData){
        statusBar_disPlayMessage("正在加载数据,请稍后再使用该功能...");
        return true;
    }
    else if(dataBlocked){
        statusBar_disPlayMessage(dataBlockedMessage);
    ...
    if(currentOpenFileType==openFileType::OFDFile||(currentOpenFileType==openFileType::FIXEDFile&&fixed.getFieldlengthtype()==0)){
        ...
        if(dialog2.getBoolFlag()){
            ...
            if(fromExcelImportFilePath.length() != 0) {
                ...
                watcherXlsxImportStatus->setFuture(future);
            } else {
                return;
            }
        }
